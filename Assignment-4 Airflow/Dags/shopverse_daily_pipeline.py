...
# Default configuration for all tasks in this DAG
DEFAULT_ARGS = {
    "owner": "data_eng",
    "retries": 1,
    "retry_delay": timedelta(minutes=5),
    "email": ["alerts@shopverse.local"],
...
def shopverse_daily_pipeline():
    """
    Main ETL pipeline for ShopVerse e-commerce data warehouse.
    Runs daily at 1 AM to process previous day's transactions.
    
    Pipeline steps:
    1. Wait for all required files (customers, products, orders)
    2. Load raw data into staging tables
    3. Transform and load into dimension and fact tables
    4. Run data quality checks
    5. Alert on low order volume days
    """

    # Get configurable threshold for low order volume alerts
    min_order_threshold = int(
        Variable.get("shopverse_min_order_threshold", default_var="10")
    )

    # Wait for all three daily data files before proceeding
    ...
    with TaskGroup(group_id="staging") as staging:
        ...
        def load_stg_products_task(products_path: str) -> int:
            """Load product catalog from CSV into staging table"""
            hook = PostgresHook(postgres_conn_id="postgres_dwh")
            inserted = 0
            conn = hook.get_conn()
            conn.autocommit = False
            with conn, conn.cursor() as cur, open(products_path, "r") as f:
                next(f)
                for line in f:
                    # Parse CSV line into product fields
                    parts = [p.strip() for p in line.strip().split(",")]
                    if len(parts) != 4:
                        continue
        ...
        def load_stg_orders_task(orders_path: str) -> int:
            """Load order transactions from JSON into staging table"""
            hook = PostgresHook(postgres_conn_id="postgres_dwh")
            inserted = 0
            conn = hook.get_conn()
            conn.autocommit = False
    ...
    # Build dimension and fact tables with transformations
    with TaskGroup(group_id="warehouse") as warehouse:
        # Create customer dimension with deduplication
        ...
        # Create product dimension with latest catalog data
        ...
        # Build fact table with currency normalization and deduplication
    ...
    def dq_check_fact_orders_not_null_keys(ds: str):
        """Ensure no orders have missing customer or product references"""
        hook = PostgresHook(postgres_conn_id="postgres_dwh")
        sql = """
    ...
    def dq_check_fact_orders_rowcount_match(ds: str):
        """Validate that all valid staging orders were loaded into fact table"""
        hook = PostgresHook(postgres_conn_id="postgres_dwh")
        valid_stg_sql = """
            SELECT COUNT(*) FROM (
                SELECT
    ...
    # Branch pipeline based on order volume to detect anomalies
    def _branch_on_volume(**context):
        """Check if daily order count is below threshold"""
        hook = PostgresHook(postgres_conn_id="postgres_dwh")
        (order_count,) = hook.get_first(
            "SELECT COUNT(*) FROM fact_orders WHERE order_date = %s;",
            parameters=(context["ds"],),
        )
        # Route to warning path if volume is unexpectedly low
    ...
    def warn_low_volume_task(ds: str):
        """Create anomaly report for days with unusually low order volume"""
        hook = PostgresHook(postgres_conn_id="postgres_dwh")
        # Get actual order count for the day
        (order_count,) = hook.get_first(
            "SELECT COUNT(*) FROM fact_orders WHERE order_date = %s;",
            parameters=(ds,),
        )
        # Prepare anomaly report directory
    ...
    # Main pipeline flow: sensors -> staging -> warehouse -> quality checks -> branching
    ...
    # Branch routes to either warning or normal completion, then both converge to end
